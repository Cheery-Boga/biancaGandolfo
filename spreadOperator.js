...
//the spread operator must be for the last, formal parameter. It gathers all the remaining arguments and puts them in an array. See below's output. In the example below, we don't have a "e" mapped to the string, 'noone', so the spread operator will account for that variable and include it in an array in the output along with the string that's mapped to the "c" parameter (on line 11). See below's example: 
...
// our second example, using the spread operator and as a ES5 function declaration: 
